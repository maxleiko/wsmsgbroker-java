    ...
    public void execute(Client client, WebSocketClient ws, JsonObject msg) {
        ...
        if (callback != null) {
            if (a.getFrom() != null && a.getMessage() != null) {
                ...
                client.emitError(new Exception("Unable to parse 'answer' message (msg.from or msg.message are undefined)"));
            }
        } else {
            client.emitError(new Exception("Unable to find answer callback for "+client.getId()+" and ack = "+a.getAck()));
        }
    }
}
